...
test("migrations 001-016 apply cleanly, reapply to zero, and serialize concurrent startup", {
  ...
  try {
    ...
    assert.deepEqual(results.map(result => result.applied.length).sort((a, b) => a - b), [0, 16])
    ...
    assert.equal(versions.rowCount, 16)
    ...
    assert.equal(
      versions.rows.find(row => row.version === "016_gift_code_sources.sql").count,
      1
    )
  } finally {
