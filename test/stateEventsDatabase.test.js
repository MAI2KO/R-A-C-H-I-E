...
test("migrations 001-022 apply cleanly, reapply to zero, and serialize concurrent startup", {
  ...
  try {
    ...
    assert.deepEqual(results.map(result => result.applied.length).sort((a, b) => a - b), [0, 22])
    ...
    assert.equal(versions.rowCount, 22)
    ...
    assert.equal(
      versions.rows.find(row => row.version === "022_active_player_account_identity_guard.sql").count,
      1
    )
  } finally {
