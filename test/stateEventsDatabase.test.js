...
test("migrations 001-012 apply cleanly, reapply to zero, and serialize concurrent startup", {
  ...
  try {
    ...
    assert.deepEqual(results.map(result => result.applied.length).sort((a, b) => a - b), [0, 12])
    ...
    assert.equal(versions.rowCount, 12)
