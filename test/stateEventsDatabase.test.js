...
test("migrations 001-017 apply cleanly, reapply to zero, and serialize concurrent startup", {
  ...
  try {
    ...
    assert.deepEqual(results.map(result => result.applied.length).sort((a, b) => a - b), [0, 17])
    ...
    assert.equal(versions.rowCount, 17)
