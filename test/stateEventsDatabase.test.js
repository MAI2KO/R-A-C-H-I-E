...
test("migrations 001-013 apply cleanly, reapply to zero, and serialize concurrent startup", {
  ...
  try {
    ...
    assert.deepEqual(results.map(result => result.applied.length).sort((a, b) => a - b), [0, 13])
    ...
    assert.equal(versions.rowCount, 13)
