...
Set `PLAYER_GIFT_CODES_ENABLED=true` to register the profile-specific `/player-register`, `/player-admin`, `/gift-code-add`, `/gift-codes`, and `/gift-codes-admin` commands. They support multiple Whiteout Survival or Kingshot characters per Discord user, one active primary character per user/profile, private account management, case-sensitive candidate submission, transactional State/Kingdom changes, soft deactivation, and explicit ownership release. `/player-admin release` is available only to the global `BOT_OWNER_IDS` allowlist; guild administration never grants that authority. PostgreSQL is canonical; the existing `/register`, `/my-info`, and `/unregister` booking identities remain unchanged in Apps Script.
...
Existing servers can grant Manage Channels to the bot's existing role and run `/setup`; removing and reinviting the bot is not required. Setup previews and reconciles the profile-specific category and five read-only parent channels one resource at a time. It reuses valid resources and configured destinations, recreates only missing expected resources, and never deletes unrelated channels. Public threads are enabled only in the gift-code and event announcement channels.
